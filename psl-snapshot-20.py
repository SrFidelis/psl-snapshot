# PSL SNAPSHOT v2.0 - 3Dview Addon - Blender 2.8
#
...
bl_info = {
    ...
    "tracker_url": "http://bit.ly/2x6oGRx",
...
def initializeSnpashotGroups(): #DEFININDO FUNÇÃO QUE CRIA AS COLEÇÕES
    try: 
...
class PSL_AddToSourceGroup(bpy.types.Operator): # CLASSE OPERADOR PARA ADICIONAR O ELEMENTO PARA A COLEÇÃO SOURCE
    ...
    bl_description = "Add element to source list"
    ...
    def execute(self, context): 
        try: 
            ...
            self.report(type={'ERROR'}, message="Error adding to the list" ) 
        return {'FINISHED'}
...
class PSL_DeleteFromSourceGroup(bpy.types.Operator): # OPERADOR PARA REMOVER O ELEMENTO DA COLEÇÃO SOURCE
    ...
    bl_description = "Delete element from source list"
...
class PSL_Snapshot_Turnoff(bpy.types.Operator): #OPERADOR PARA FINALIZAR O PSL
    bl_idname = "object.psl_snp_turnoff"
    bl_label = "Turn Off"    
    bl_description = "Turn off addon"
